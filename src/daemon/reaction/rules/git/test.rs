...
fn test_git_rule_matches() {
  ...
  for cmd in commands {
    ...
    assert!(GitRule.matches(&event), "Should match '{}'", cmd);
  }
}
...
fn test_git_reaction() {
  ...
  assert!(GitRule.react(&event).is_some());
  assert!(GitRule.react(&event).unwrap().contains("Recording today's mistake for future reference."));
  assert!(GitRule.react(&event).unwrap().contains("(◠‿ o )"));
}
